...
*原生界面截图。图中为自检样例，不是真实模型输出。选中历史行切换大图，右侧箭头打开对应 SVG。*
...
`Package.swift` 声明 tools version 5.9，测试则依赖 Swift Testing；当前验证工具链为 Apple Swift 6.4。仓库提供 macOS GitHub Actions 工作流，运行导出边界测试、Swift 测试及应用构建；没有独立 lint 脚本。托管运行结果需在推送后验证。`.build/`、`dist/` 和 `artifacts/` 不纳入 Git。
